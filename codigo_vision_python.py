...
#mejor sin buenas ni malas, con esto se puede entregar
import os
import cv2
import torch
...
# # Clasificador buena/mala (EfficientNet)
...
# # Modelos específicos por familia PARA CLASIFICACIÓN BUENA/MALA
# MODELS_BY_FAMILY = {
#     "argollas": r"C:\Users\Sebastian\OneDrive\Escritorio\Vision\modelo_buenas_malas\argollas_buenas_malas2.pt",
#     "ochos": r"C:\Users\Sebastian\OneDrive\Escritorio\Vision\modelo_buenas_malas\ochos_buenas_malas2.pt",
#     "tensores": r"C:\Users\Sebastian\OneDrive\Escritorio\Vision\modelo_buenas_malas\tensores_buenas_malas2.pt",
#     "zetas": r"C:\Users\Sebastian\OneDrive\Escritorio\Vision\modelo_buenas_malas\zetas_buenas_malas2.pt"
# }
...
# MIN_CONFIDENCE_THRESHOLD = 0.70  # 70% de confianza mínima
...
# def build_effnet_binary(device, architecture='complex'):
#     """
#     🔧 Construye EfficientNet con arquitectura correcta.
#     architecture: 'simple' o 'complex' (debe coincidir con el entrenamiento)
#     """
...
#     num_ftrs = model.classifier[1].in_features  # 1280 para EfficientNet-B0
    
#     if architecture == 'complex':
#         # 🔥 Arquitectura COMPLEJA (coincide con modelos entrenados con código nuevo)
#         model.classifier = nn.Sequential(
#             nn.Dropout(p=0.3),
#             nn.Linear(num_ftrs, 256),           # 1280 → 256
#             nn.BatchNorm1d(256),
...
#         )
#     else:
#         # Arquitectura SIMPLE (modelos antiguos)
#         model.classifier = nn.Sequential(
#             nn.Dropout(p=0.2, inplace=True),
#             nn.Linear(num_ftrs, 2)              # 1280 → 2
#         )
    
#     model = model.to(device).eval()
#     return model, weights.transforms()

# def smart_load_state_dict(model, state_dict_path, strict=False):
#     """🔧 FIXED: Compatible con PyTorch 2.6+ y arquitecturas complejas"""
#     try:
...
#     except Exception as e:
#         print(f"      ⚠️  Intento alternativo de carga...")
#         sd = torch.load(state_dict_path, map_location="cpu")
    
#     # Si el checkpoint es un diccionario con 'model_state_dict'
#     if isinstance(sd, dict) and 'model_state_dict' in sd:
#         sd = sd['model_state_dict']
    
#     keys = list(sd.keys())
#     if len(keys) == 0:
#         raise RuntimeError("El state_dict está vacío.")
    
#     # Manejar prefijo 'model.'
#     has_model_prefix = all(k.startswith("model.") for k in keys)
#     needs_model_prefix = any(k.startswith("model.") for k in model.state_dict().keys())
    
#     if has_model_prefix and not needs_model_prefix:
#         sd = {k.replace("model.", "", 1): v for k, v in sd.items()}
#     elif (not has_model_prefix) and needs_model_prefix:
...
#         self.root.title("🔥 Sistema de Inspección - sebastianmolina10-blip 🔥")
#         self.root.geometry("1500x850")
#         self.root.configure(bg="#f0f0f0")
...
#         self.last_detected_area = None
#         self.last_detected_size_category = None
#         self.last_detected_classification = None
...
#         style.configure("Area.TLabel", font=('Consolas', 16, 'bold'), foreground="#0288d1")
#         style.configure("Counter.TLabel", font=('Consolas', 14, 'bold'), foreground="#1B5E20")
#         style.configure("Confidence.TLabel", font=('Consolas', 12, 'bold'), foreground="#FF6F00")
...
#         counter_frame = ttk.Frame(parent, relief=tk.SOLID, borderwidth=2, padding=10)
#         counter_frame.pack(pady=5, padx=20, fill=tk.X)
#         ttk.Label(counter_frame, text="📊 CONTADOR DE FAMILIAS (BUENAS):", 
#                   font=('Helvetica', 12, 'bold')).pack()
#         ttk.Label(counter_frame, textvariable=self.family_counter_var, 
#                   style="Counter.TLabel", justify=tk.LEFT).pack(pady=2)
...
#                   font=('Helvetica', 12, 'bold')).pack()
#         ttk.Label(parent, textvariable=self.identificacion_completa_var, 
#                   style="IDFinal.TLabel", anchor="center").pack(pady=5)
#         ttk.Label(parent, textvariable=self.confidence_var, 
#                   style="Confidence.TLabel", anchor="center").pack(pady=2)
...
#     def _restore_last_detection_ui(self):
#         if self.last_detected_family:
#             if self.last_detected_classification:
#                 family_text = f"{self.last_detected_family.upper()}_{self.last_detected_classification.upper()}"
#                 conf_text = f"Confianza: {self.last_detected_confidence:.1%}"
#             else:
#                 family_text = f"{self.last_detected_family.upper()}"
#                 conf_text = "Confianza: -"
...
#             print(f"\n{'='*70}")
#             print(f"🔥 CARGANDO MODELOS - sebastianmolina10-blip")
#             print(f"📅 Fecha: 2025-11-15 14:11:09 UTC")
#             print(f"{'='*70}\n")
            
#             print(f"📦 Cargando YOLO desde: {YOLO_FAMILIAS_PATH}")
#             self.yolo = YOLO(YOLO_FAMILIAS_PATH)
...
#             self.family_counts = {name: 0 for name in self.family_names}
#             self._update_family_counter_display()
#             print(f"   ✅ YOLO cargado: {len(self.yolo.names)} clases detectadas → {self.family_names}")

#             # 🔧 Cargar clasificadores con arquitectura COMPLEJA
#             print(f"\n📦 Cargando clasificadores EfficientNet (arquitectura compleja)...")
#             _, self.clf_tfms = build_effnet_binary(self.device, architecture='complex')
#             loaded_classifiers = 0
            
#             for familia, model_path in MODELS_BY_FAMILY.items():
#                 if os.path.exists(model_path):
#                     print(f"\n   📂 Familia: '{familia}'")
#                     print(f"      Ruta: {model_path}")
#                     try:
#                         # 🔥 Usar arquitectura COMPLEJA (256 neuronas intermedias)
#                         modelo, _ = build_effnet_binary(self.device, architecture='complex')
#                         smart_load_state_dict(modelo, model_path, strict=False)
#                         self.classifiers[familia] = modelo
...
#             print(f"\n{'='*70}")
#             status_msg = f"✅ Sistema listo: YOLO ({len(self.yolo.names)} clases) + {loaded_classifiers}/{len(MODELS_BY_FAMILY)} clasificadores"
#             print(status_msg)
#             print(f"{'='*70}\n")
            
#             self.status.config(text=status_msg)
            
#             if loaded_classifiers < len(MODELS_BY_FAMILY):
#                 print(f"⚠️  ADVERTENCIA: Solo {loaded_classifiers} de {len(MODELS_BY_FAMILY)} clasificadores disponibles")
#                 print(f"   Las familias sin modelo clasificarán como 'mala' por defecto\n")
        
#         except Exception as e:
#             error_msg = f"❌ Error crítico al cargar modelos: {e}"
...
#                 self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
#                 self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
#                 print(f"✅ Cámara abierta (index={idx}, 1280x720)")
#                 self.status.config(text=f"✅ Sistema completo: Cámara + Modelos cargados")
#                 return
#         print(f"❌ No se pudo abrir ninguna cámara")
#         self.status.config(text="❌ Error: No se pudo abrir la cámara")

#     def _classify_good_bad(self, bgr_crop, family):
#         """Clasifica pieza como buena/mala con confianza"""
#         if not family or family not in self.classifiers or bgr_crop.size == 0:
#             print(f"      ⚠️  No hay clasificador para '{family}', marcando como MALA")
#             return "mala", 0.0
#         try:
#             modelo = self.classifiers[family]
#             rgb = cv2.cvtColor(bgr_crop, cv2.COLOR_BGR2RGB)
#             pil = Image.fromarray(rgb)
#             x = self.clf_tfms(pil).unsqueeze(0).to(self.device)
...
#             label = "buena" if pred.item() == 0 else "mala"
...
#                 print(f"\n{'='*60}")
#                 print(f"✅ PIEZA DETECTADA EN CENTRO")
#                 print(f"   Familia: {self.detected_family}")
#                 print(f"   Área: {self.final_area:,.0f} px²")
#                 print(f"   Tamaño: {self.final_size_category}")
                
#                 if self.mode == "familias":
#                     print(f"   Clasificando buena/mala...")
#                     current_crop = frame[y1:y2, x1:x2]
#                     label, conf = self._classify_good_bad(current_crop, self.detected_family)
                    
#                     self.final_classification = label
#                     self.final_confidence = conf
#                     self.classification_done = True
                    
#                     print(f"   🎯 Resultado: {label.upper()} (confianza: {conf:.2%})")
                    
#                     if conf < MIN_CONFIDENCE_THRESHOLD and label == "buena":
#                         print(f"   ⚠️  Confianza < {MIN_CONFIDENCE_THRESHOLD:.0%}, marcando como MALA por seguridad")
#                         self.final_classification = "mala"
...
#                 if self.mode == "familias":
#                     self.last_detected_classification = self.final_classification
#                     self.last_detected_confidence = self.final_confidence
                    
#                     if self.final_classification == "buena" and self.last_detected_family in self.family_names:
#                         comando_a_enviar = self.last_detected_family
#                         self.family_counts[self.last_detected_family] += 1
#                         self._update_family_counter_display()
#                         print(f"→ Pieza BUENA ({self.final_confidence:.2%}): Enviando '{comando_a_enviar}'")
#                     else:
#                         print(f"→ Pieza MALA: No se envía comando ni se cuenta")

#                 elif self.mode == "tamanos":
#                     if self.final_size_category:
#                         comando_a_enviar = self.final_size_category
#                         if self.final_size_category == 'S':
#                             self.count_S += 1
#                         elif self.final_size_category == 'M':
...
#                 x1, y1, x2, y2 = self.locked_box
#                 if self.mode == "familias":
#                     if self.classification_done and self.final_classification:
#                         color = (0, 255, 0) if self.final_classification == "buena" else (0, 0, 255)
#                     else:
#                         color = (255, 165, 0)
#                 else:
#                     color = (0, 255, 0)
